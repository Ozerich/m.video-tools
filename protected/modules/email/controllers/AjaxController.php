...
class AjaxController extends Controller
{
    ...
    public function actionSubmit_block()
    {
        ...
        if ($is_catalog) {
            ...
            if ($model->type == 'product') {
                ...
                $model->product_name_additional = isset($request_data['name_additional']) ? $request_data['name_additional'] : '';
                ...
                $model->utm_content = isset($request_data['utm_content']) ? $request_data['utm_content'] : null;
				$model->alt = isset($request_data['alt']) ? str_replace('"', '‘’',$request_data['alt']) : '';
            }
            ...
            if ($model->type == 'banner') {
                ...
                $model->alt = str_replace('"', '‘’',isset($request_data['alt']) ? $request_data['alt'] : '');

                $areas = Yii::app()->request->getPost('banner_areas', array());
                if ($areas) {
                    $model->banner_url = '';
                    foreach ($areas as $area) {
                        $model_areas[$area['coords']] = array('url' => $area['url'], 'utm_content' => $area['utm_content'], 'alt' =>  str_replace('"', '‘’',$area['alt']));
                    }
                }
    ...
    public function actionSendPreview()
    {
        ...
        $preview = $this->render('/letter/' . $model->layout . '/main', array('letter' => $model), true);
    ...
    public function actionDownload()
    {
        ...
        foreach ($regions as $region) {
            ...
            $html = iconv('UTF-8', 'Windows-1251', $this->render('/letter/' . $model->layout . '/main', array('letter' => $model, 'region' => $region, 'encoding' => 'Windows-1251'), true));
