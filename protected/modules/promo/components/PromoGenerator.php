...
class PromoGenerator
{
    private function getSamplePage($type)
    {
        if ($type == PromoForm::TYPE_FULL) {
            return 'http://magnitogorsk.mvideo.ru/ipad-air/';
        } else if ($type == PromoForm::TYPE_NOT_FULL) {
            return "http://magnitogorsk.mvideo.ru/iphone-5s/";
        }
    ...
    {
        $html = str_replace('href="/', 'href="http://magnitogorsk.mvideo.ru/', $html);
        $html = str_replace('src="/', 'src="http://magnitogorsk.mvideo.ru/', $html);
    ...
    public function createPromo($name, $type)
    {
        ...
        mkdir($dir . '/pics/a/' . $name, 0777, true);
